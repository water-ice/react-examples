 ...
 * @return {Object}
 */
export function parseUrl(windowHash = location.pathname) { //解析url
	let path = [];
	const query = {};
 ...
 * @return {[type]}          新的url
 */
export function hashUrl(paramObj, url) {
	let paramArray = [];
 ...
 * @return {String}
 */
export function getUrlParam(key, urlInfo) {
 ...
 * 记忆滚动监听需要初始化
 */
export function initLink(event) {
	const url = event.currentTarget.getAttribute('href');
	_global.scroll[url] = 0;
