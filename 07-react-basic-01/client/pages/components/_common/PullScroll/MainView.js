...
class MainView extends Component {
	constructor(props, context) {
		...
		this.privatePulling = this.privatePulling.bind(this);
		this.privatePullEnd = this.privatePullEnd.bind(this);
		this.privateChangeStatus = this.privateChangeStatus.bind(this);
	}
	componentWillMount() {
	...
	componentDidMount() {
		this.pullContainer = (this.wrapper) ? document.querySelector(this.wrapper) : document.body;
	...
	handleMove(event) {
		const eTouchScreenY = event.touches ? event.touches[0].screenY : event.screenY; // 也可使用clientY
		/**
		 * 处理微信露底问题
		 * 
